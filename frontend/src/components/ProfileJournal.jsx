...
//       variants={fadeInUp}
//       className="bg-white/50 rounded-2xl shadow-sm p-6 mb-6 hover:shadow-lg transition-shadow duration-300"
//     >
...
//         animate="visible"
//         variants={fadeInUp}
//         className="bg-white/20 shadow-lg rounded-2xl p-6 mb-8"
//       >
//         <Calendar 
...
function JournalEditor({ entry, onSave, onCancel }) {
  ...
  return (
    <motion.div
      ...
      exit="hidden"
      variants={fadeInUp}
      className="bg-white/50 backdrop-blur-lg rounded-2xl p-6 mb-8 shadow-lg"
    >
      <motion.div className="flex justify-between items-center mb-4"
        initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: 0.2, duration: 0.4 } }}
      >
        <h2 className="text-2xl font-calligraphy text-gray-900">
          {entry ? 'Edit Entry' : 'New Entry'}
        </h2>
        <button onClick={onCancel} className="text-gray-400 hover:text-gray-700 text-2xl">×</button>
      </motion.div>
      ...
      >
        <motion.input variants={fadeInUp}
          ...
          value={title}
          onChange={e => setTitle(e.target.value)}
          className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-300"
        />
        <motion.textarea variants={fadeInUp}
          placeholder="What's on your mind?"
          value={body}
...
function JournalEntryCard({ entry, onEdit, onDelete }) {
  ...
  return (
    <motion.div
      initial="hidden" animate="visible"
      variants={fadeInUp}
      className="bg-white/50 backdrop-blur-lg rounded-2xl p-6 mb-6 hover:shadow-lg transition-shadow"
    >
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-semibold text-gray-800">{entry.title || 'Untitled'}</h3>
        <span className="text-sm text-gray-600">{formattedDate}</span>
      </div>
      <div className="flex items-center gap-3 mb-4">
        <span className="text-2xl animate-pulse">{entry.mood}</span>
        {entry.tags.map(tag => (
          <span key={tag} className="bg-emerald-100 text-emerald-700 text-xs px-2 py-1 rounded-full">{tag}</span>
        ))}
      </div>
      <p className="text-gray-800 mb-4 whitespace-pre-line leading-relaxed">{entry.body}</p>
      <div className="flex justify-end gap-4">
        <button onClick={() => onEdit(entry)} className="text-emerald-600 hover:underline">Edit</button>
        <button onClick={() => onDelete(entry._id)} className="text-red-600 hover:underline">Delete</button>
      </div>
...
export default function ProfileJournals() {
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={fadeIn} className="space-y-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-calligraphy text-white drop-shadow-lg">My Journal</h2>
        <button
          onClick={() => { setEditingEntry(null); setShowEditor(true); }}
      ...
      <div className="bg-white/20 backdrop-blur-lg rounded-2xl p-6 mb-8">
        <Calendar
          prevLabel={<ChevronLeft className="text-emerald-600 hover:scale-110 transition" />}
          nextLabel={<ChevronRight className="text-emerald-600 hover:scale-110 transition" />}
          onClickDay={date => {
            const clicked = entries.find(
              e => new Date(e.date).toDateString() === date.toDateString()
            );
          ...
          tileContent={({ date, view }) =>
            view === 'month' && dateSet.has(date.toDateString()) ? (
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="h-2 w-2 bg-emerald-600 rounded-full mx-auto mt-1" />
            ) : null
          }
          tileClassName={({ date, view }) =>
            `react-calendar__tile text-gray-800 bg-transparent hover:bg-emerald-50 transition-colors ${view === 'month' && dateSet.has(date.toDateString()) ? 'bg-yellow-200' : ''}`
          }
      ...
      {loading ? (
        <p className="text-gray-200">Loading entries...</p>
      ) : (
        entries.map(entry => (
          <JournalEntryCard
            key={entry._id}
