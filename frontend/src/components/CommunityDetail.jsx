...
export default function CommunityDetail({ communityId, onBack }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        // 1) Community
        let res = await fetch(`https://empathaiv2-backend.onrender.com/communities/${id}`);
        if (!res.ok) throw new Error((await res.json()).error);
        const communityData = await res.json();

        // 2) Posts (always fetch, but we’ll conditionally render)
        res = await fetch(`https://empathaiv2-backend.onrender.com/posts/community/${id}`);
        if (!res.ok) throw new Error((await res.json()).error);
  ...
  const handleJoin = async () => {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `https://empathaiv2-backend.onrender.com/communities/${id}/join`,
      { method: "POST", headers: { Authorization: `Bearer ${token}` } }
    );
    if (!res.ok) throw new Error((await res.json()).error);
    setCommunity(c => ({ ...c, members: [...c.members, currentUser.id] }));
  ...
  const handleRequestToJoin = async () => {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `https://empathaiv2-backend.onrender.com/communities/${id}/request`,
      { method: "POST", headers: { Authorization: `Bearer ${token}` } }
    );
    if (!res.ok) throw new Error((await res.json()).error);
    alert("Join request sent");
  ...
  const handleLeave = async () => {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `https://empathaiv2-backend.onrender.com/communities/${id}/leave`,
      { method: "POST", headers: { Authorization: `Bearer ${token}` } }
    );
    if (!res.ok) throw new Error((await res.json()).error);
    setCommunity(c => ({
  ...
  const handleRequest = async (userId, action) => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `https://empathaiv2-backend.onrender.com/communities/${id}/approve`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
